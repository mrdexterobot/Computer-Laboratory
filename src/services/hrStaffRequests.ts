...
const DEPT_NAME = 'Computer Laboratory';
export type ComlabRoleType = HrStaffIntegrationRoleCode;
...
export async function createHrStaffRequest(payload: {
  ...
  const poolPath = `hr_staff_directory?employee_no=eq.${encodeURIComponent(poolKey)}&select=id&limit=1`;
  let staffRows = await sbGet<{ id: number }>(poolPath);
  if (!staffRows.length) {
    assertConfig();
    const res = await fetch(`${resolveSupabaseUrl()}/rest/v1/hr_staff_directory`, {
      method: 'POST',
    ...
    if (!res.ok) {
      ...
      const duplicate = res.status === 409 || errBody.includes('23505');
      if (!duplicate) {
        throw new Error(errBody || `Request failed (${res.status})`);
      }
    }
    staffRows = await sbGet<{ id: number }>(poolPath);
  }
